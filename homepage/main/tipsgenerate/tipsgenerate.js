...
function filterTipsBySearch(name, list) {
  ...
  return list.filter((item, index, arr) => {
    const gameName = String(item).toLowerCase();

    // cari line sebelum / selepas nama game
    const prev = String(arr[index - 1] || '').toLowerCase();
    const next = String(arr[index + 1] || '').toLowerCase();
